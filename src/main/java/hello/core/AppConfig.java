...
//    // 단일 책임 원칙이 지켜지는 것.
    ...
    // 위처럼 만드는 게 초안.
    // 위처럼 만들어도 물론 좋은 코드임.
    // 하지만 여기서 더 리팩토링 여지가 있음.
    // 어떤 인터페이스에 어떤 구현체를 쓸지, 한눈에 딱딱 짝지어서 볼 수 있으면 더 편하겠지.
    // 그리고 생성자마다 중복된 구현체도 있어. 예를 들어 MemberServiceImpl 에서도 MemoryMemberRepository 를 쓰고 OrderServiceImpl 에서도 MemoryMemberRepository 를 써.
    // 없는 것도 있어. DiscountPolicy 는 FixDiscountPolicy 를 쓸지 RateDiscountPolicy 를 쓸지 결정해주는 메소드도 없음.
    // 이거를 한번에 해결할 수 있는 방법이 있음.
    // 각 인터페이스마다 메소드를 하나씩 만들고, 그 인터페이스는 이 구현체를 쓴다고 return 해 주는 메소드를 만들면 됨.
    ...
    public MemberRepository memberRepository() {
        // 이렇게 해두니까 메소드만 봐도 추상 클래스와 구현 클래스가 한눈에 짝지어서 보이지.
        // 위에 return type 보면 인터페이스 보이고, 그 인터페이스 역할을 누가 수행할지는 아래 return 구문 보면 보이고.
